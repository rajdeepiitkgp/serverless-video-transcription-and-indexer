...
Operational context for coding agents (GitHub Copilot, etc.). This is **not** a copy of
the plan — the reviewed contract for the build is
[IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md) (v5). Read the relevant section before
building anything; do not deviate from it silently.

**The plan is frozen** (as is `requirement.md`): both are read-only contract documents.
Never edit them. Corrections or scope changes are recorded as ADRs in `docs/adr/`
referencing the section they amend.
...
fully green, webapi failed only on the assert's too-short 90s window. #4: webapi
...
> Maintenance note: [CLAUDE.md](CLAUDE.md) is the canonical agent-context file; this
> file mirrors it minus Claude-Code-specific tooling. When CLAUDE.md changes, update
